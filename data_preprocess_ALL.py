...
def generate_affinity_rawdata(_filename, Flag=False):
...
def generate_neutralization_rawdata(_filename, Flag=False):
    dataset = pd.read_csv("./data/%s" % _filename, encoding='gbk')
    # 读取抗体对应的名称，亦即“Name”
    antibody_name = dataset[['Name']]
    antibody_list = np.array(antibody_name).squeeze().tolist()
...
if __name__ == '__main__':
    ...
    aff_merge = pd.concat([aff_ori, aff_var], axis=0)
    ...
    affE_merge = pd.concat([affE_ori, affE_var], axis=0)
    ...
    Neu_merge = pd.concat([Neu_ori, Neu_var], axis=0)
