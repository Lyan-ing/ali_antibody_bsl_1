...
def get_feature(_list):
    ...
    for seq in tqdm(_list):
        ...
        if len(seq) >1:
            i=i+1
        for cur_seq in seq:
            token_ids = torch.tensor([tokenizer.encode(cur_seq)])
            output = model(token_ids)
            pooled_output = output[1]
            cur_feature.append(pooled_output)
        feature.append(torch.cat(cur_feature, dim=0).mean(dim =0).tolist())
        # else:
...
if __name__ == '__main__':
    # 提取特征时全部提取
    df = pd.read_csv('./data/Merge_aff_train_noE.csv', sep=',')
    sequence = df['Sequence'].astype(str).tolist()
    df = get_feature(sequence)
    df.to_csv('./data/Merge_feature_aff_noE.csv', index=False)
    df2 = pd.read_csv('./data/Merge_Neu_train_noE.csv', sep=',')
    sequence = df2['Sequence'].astype(str).tolist()
    df2 = get_feature(sequence)


    df2.to_csv('./data/Merge_feature_neu_noE.csv', index=False)
